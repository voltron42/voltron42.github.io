namespace("luigis-music.Hertz",{},() => {
    const hertz = {
        "C":[16.35, 32.7, 65.41, 130.81, 261.63, 523.25, 1046.50, 2093.00, 4186.01],
        ...
        "F#/Gb":[23.12, 46.25, 92.5, 185.00, 369.99, 739.99, 1479.98, 2959.96, 5919.91],
    ...
    return {
        donut: function(pitch, octave) {
            const note = hertz[pitch];
            if (note) {
                return hertz[pitch][octave];
            } else {
                let notes = Object.keys(hertz).filter((key) => (key.indexOf(pitch) >= 0));
                if (notes.length >= 0) {
                    return hertz[notes[0]][octave];
                } else {
                    throw {
